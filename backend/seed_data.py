...
async def seed():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # ── Users ────────────────────────────────────────────────────
        users = [
            User(name="Asha Menon",   email="asha@ecosphere.test",   department_id=ops.id,        total_xp=0, total_points=0),
            User(name="Rahul Varma",  email="rahul@ecosphere.test",  department_id=logistics.id,  total_xp=0, total_points=0),
            User(name="Divya Nair",   email="divya@ecosphere.test",  department_id=corporate.id,  total_xp=0, total_points=0),
            User(name="Arjun Pillai", email="arjun@ecosphere.test",  department_id=facilities.id, total_xp=0, total_points=0),
        ]
        db.add_all(users)
        await db.flush()
